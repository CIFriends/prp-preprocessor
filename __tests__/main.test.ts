...
test("pushChanges should commit and push changes successfully", async () => {
  await pushChanges(gitMock, inputParams, "Test commit message");
  expect(gitMock.addConfig).toHaveBeenCalledTimes(2);
  expect(gitMock.addConfig).toHaveBeenNthCalledWith(1, "user.name", inputParams.userName, undefined, "worktree");
  expect(gitMock.addConfig).toHaveBeenNthCalledWith(2, "user.email", inputParams.userEmail, undefined, "worktree");
